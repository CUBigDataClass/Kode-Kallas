...
def addExtraData(repoList):
    threadPool = Pool(10)
    updatedRepoList = threadPool.map(processRepoList, repoList)
    return updatedRepoList

def processRepoList(repoItem):
    repoItem['languages_list'] = get(getLanguages, repoItem['languages_url'])
    repoItem['contributors_list'] = get(getContributorsList, repoItem['contributors_url'])
    repoItem['issues_list'] = get(getIssuesList, repoItem['issues_url'][:-9])
    return repoItem
